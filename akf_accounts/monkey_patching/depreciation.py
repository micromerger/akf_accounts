...
def _make_journal_entry_for_depreciation(
	...
	# print(f"sch_start_idx: {sch_start_idx}")
	# print(f"sch_end_idx: {sch_end_idx}")
	# print(f"depr_schedule.journal_entry: {depr_schedule.journal_entry}")
	# print(f"depr_schedule.schedule_date: {depr_schedule.schedule_date}")
	# print(f'date: {date}')
	...
	# Income Account
	asset_income_credit_entry = {}
	asset_income_account = get_asset_income_account(asset.company, asset.asset_category)
	if(asset_income_account):
		asset_income_credit_entry.update({
			"account": asset_income_account,
			"credit_in_account_currency": depr_schedule.depreciation_amount,
			"reference_type": "Asset",
			"reference_name": asset.name,
			"cost_center": depreciation_cost_center,
	...
	# Nabeel created on 29-10-2025
	# Customized Code:- 
	# Designated Asset Fund Account
	asset_fund_debit_entry = {}
	asset_designated_fund_account = get_asset_designated_fund_account(asset.company, asset.asset_category)
	
	if(asset_designated_fund_account):
		asset_fund_debit_entry.update({
			"account": asset_designated_fund_account,
			"debit_in_account_currency": depr_schedule.depreciation_amount,
			"reference_type": "Asset",
			"reference_name": asset.name,
			"cost_center": depreciation_cost_center,
	...
	if(asset_income_credit_entry): je.append("accounts", asset_income_credit_entry) # Nabeel created on 29-10-2025
	if(asset_fund_debit_entry): je.append("accounts", asset_fund_debit_entry) # Nabeel created on 29-10-2025
...
def get_asset_income_account(company, asset_category):
	...
	"""if(not account):
		frappe.throw(
      		f"Please set `<b>Income Account (cr)</b>` of asset category <b>{asset_category}</b>. ", 
        	title="Missing Information"
        )
	return account"""
