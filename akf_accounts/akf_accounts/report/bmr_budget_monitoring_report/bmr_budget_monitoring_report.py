...
def get_final_data(dimension, dimension_items, filters, period_month_ranges, data, DCC_allocation):
	for account, monthwise_data in dimension_items.items():
			...
			for relevant_months in period_month_ranges:
				...
				for month in relevant_months:
					if monthwise_data.get(year[0]):
						...
						for i, fieldname in enumerate(["target", "actual"]):
				...
				# Variance % (safe from zero division)
				period_data[3] = (period_data[2] / period_data[0] * 100) if period_data[0] else 0

				# Allocation adjustment
				if DCC_allocation:
					for i in range(2):  # target and actual
						period_data[i] = period_data[i] * (DCC_allocation / 100)
		...
		totals[3] = (totals[2] / totals[0] * 100) if totals[0] else 0
...
# 					if monthwise_data.get(year[0]):
# 						month_data = monthwise_data.get(year[0]).get(month, {})
# 						for i, fieldname in enumerate(["target", "actual", "variance", "variance_per"]):
# 							value = flt(month_data.get(fieldname))
# 							period_data[i] += value
# 							totals[i] += value
...
def get_columns(filters):
	...
	if filters["period"] != "Yearly":
		for label in [_("Total Budget"), _("Total Actual"), _("Total Variance"), _("Total Variance Per")]:
			columns.append(
				{"label": label, "fieldtype": "Float", "fieldname": frappe.scrub(label), "width": 150}
			)
