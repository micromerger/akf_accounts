...
def get_gl_entries(filters, accounting_dimensions):
	...
	if accounting_dimensions:
		accounting_dimensions.append("(select donor_name from `tabDonor` where name = gl.donor) as donor_name")
	...
	gl_entries = frappe.db.sql(
		...
		select
			...
			cost_center, project, (select project_name from `tabProject` where name=gl.project) as project_name, {transaction_currency_fields}
...
def get_columns(filters):
	...
	columns += [
		...
		{"label": _("Project Name"), "options": "Project", "fieldname": "project_name", "width": 100},
	]
	...
	if filters.get("include_dimensions"):
		for dim in get_accounting_dimensions(as_list=False):
			...
			if(dim.fieldname == "donor"):
				columns.append(
					{"label": _("Donor Name"), "options": "", "fieldname": "donor_name", "width": 100}
				)
		columns.append(
