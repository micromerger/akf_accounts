...
def get_data(filters):
    ...
    if donor:
        where_conditions.append("db.donor = %(donor)s")
        query_params["donor"] = donor

    where_clause = " AND ".join(where_conditions)

    income_data = frappe.db.sql(
        f"""
        SELECT db.income_type, SUM(db.amount) AS amount, d.donation_cost_center, pd.service_area, pd.subservice_area, pd.product, pd.project_id, db.donor
        ...
        GROUP BY db.donor, db.income_type, d.donation_cost_center, pd.service_area, pd.subservice_area, pd.product, pd.project_id
        """,
        query_params,
        as_dict=True
    )
    ...
    for entry in income_data:
        key = (company, entry.get("donor"), entry.get("donation_cost_center"), entry.get("service_area"), entry.get("subservice_area"), entry.get("product"), entry.get("project_id"))
    ...
    if donor:
        gl_where_conditions.append("donor = %(donor)s")
        gl_query_params["donor"] = donor

    gl_where_clause = " AND ".join(gl_where_conditions)

    gl_entries = frappe.db.sql(
        f"""
        SELECT donor, account, debit, credit, cost_center, service_area, subservice_area, product, project, posting_date, voucher_type, voucher_no
        FROM `tabGL Entry`
        WHERE {gl_where_clause}
        AND project != '' AND donor != ''
        GROUP BY donor, cost_center, service_area, subservice_area, product, project
        """,
        gl_query_params,
        as_dict=True
    )
    ...
    for entry in gl_entries:
        key = (company, entry.get("donor"), entry.get("cost_center"), entry.get("service_area"), entry.get("subservice_area"), entry.get("product"), entry.get("project"))
        balance_data.setdefault(key, {
            'opening_balance': 0, 'receivable_balance': 0,
            'closing_balance': 0, 'closing_receivable': 0,
            'inter_fund_sent': 0, 'inter_fund_received': 0,
    ...
    report_data = [
        [
            company, donor, cost_center, service_area, subservice_area, product, project,
        ...
        for (company, donor, cost_center, service_area, subservice_area, product, project), balances in balance_data.items()
    ]
