...
def get_query_result(filters):
    ...
    result = frappe.db.sql(
        """
        SELECT 
			gle.company, gle.donor, gle.account,gle.cost_center,gle.service_area,gle.subservice_area,proj.project_name, gle.project, SUM(gle.credit)-SUM(gle.debit)
        FROM 
            `tabGL Entry` gle
        LEFT JOIN 
        ...
        WHERE
            gle.donor != '' AND acc.root_type = 'Equity' {0} and gle.docstatus = 1
        Group By
            gle.donor, gle.account,gle.cost_center,gle.project""".format(conditions if conditions else ""
        ),
        filters,
        as_dict=0,
    )
