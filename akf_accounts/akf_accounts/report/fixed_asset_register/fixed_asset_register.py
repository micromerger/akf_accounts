...
def get_conditions(filters):
    ...
    if filters.filter_based_on == "Date Range":
        ...
        validate_fiscal_year(
            fiscal_year, filters.from_fiscal_year, filters.to_fiscal_year
        )
    ...
    if status:
        ...
        conditions["status"] = (
            operand,
            ["Sold", "Scrapped", "Capitalized", "Decapitalized"],
        )
...
def get_data(filters):
    ...
    fields = [
        ...
        "asset_name",
        "status",
        "custom_source_of_asset_acquistion",
        "custom_type_of_asset",
        "custom_asset_specification",
        "custom_reason_for_asset_scrap",
        "department",
        "company",
        "cost_center",
    ...
    for asset in assets_record:
        ...
        row = {
            ...
            "status": asset.status,
            "custom_source_of_asset_acquistion": asset.custom_source_of_asset_acquistion,
            "custom_type_of_asset": asset.custom_type_of_asset,
            "custom_asset_specification": asset.custom_asset_specification,
            "custom_reason_for_asset_scrap": asset.custom_reason_for_asset_scrap,
            ...
            "datasets": [
                {
                    "name": _("Asset Value"),
                    "values": [
                        flt(d.get("asset_value"), 2) for d in labels_values_map.values()
                    ],
                },
                {
                    "name": _("Depreciatied Amount"),
                    "values": [
                        flt(d.get("depreciated_amount"), 2)
                        for d in labels_values_map.values()
                    ],
                },
            ],
        },
...
def get_asset_depreciation_amount_map(filters, finance_book):
    ...
    query = (
        ...
        .where(
            gle.account
            == IfNull(
                aca.depreciation_expense_account, company.depreciation_expense_account
            )
        )
        .where(gle.debit != 0)
    ...
    if finance_book:
        query = query.where(
            (gle.finance_book.isin([cstr(finance_book), ""]))
...
def get_group_by_data(
    ...
    for a in assets:
        ...
        idx = ([i for i, d in enumerate(data) if a[group_by] == d[group_by]] or [None])[
            0
        ]
        if idx is None:
            data.append(a)
...
def get_columns(filters):
    ...
    return [
        ...
        {
            "label": _("Source of Asset Acquistion"),
            "fieldtype": "Select",
            "fieldname": "custom_source_of_asset_acquistion",
            "width": 140,
        },
        {
            "label": _("Type of Asset"),
            "fieldtype": "Select",
            "fieldname": "custom_type_of_asset",
            "width": 140,
        },
        {
            "label": _("Asset Specification"),
            "fieldtype": "Small Text",
            "fieldname": "custom_asset_specification",
            "width": 140,
        },
        {
            "label": _("Reason for Asset Scrap"),
            "fieldtype": "Select",
            "fieldname": "custom_reason_for_asset_scrap",
            "width": 140,
        },
        {
