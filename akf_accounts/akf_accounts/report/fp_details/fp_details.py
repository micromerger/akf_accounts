...
def get_columns():
    return [
        ...
        _("Donor Type") + ":Link/Donor Type:140",
...
def get_fund_data(filters, fund_type):
    ...
    if filters.get("subservicearea"):
        conditions += " AND gle.subservicearea = %(subservicearea)s"
        params["subservicearea"] = filters.subservicearea
    if filters.get("project"):
        conditions += " AND gle.project = %(project)s"
        params["project"] = filters.project
    if filters.get("donor"):
        conditions += " AND gle.donor = %(donor)s"
        params["donor"] = filters.donor
    if filters.get("donor_type"):
        conditions += " AND gle.donor_type = %(donor_type)s"
        params["donor_type"] = filters.donor_type

    gle_entries = frappe.db.sql(f"""
        SELECT
            gle.company, gle.cost_center, sa.name as service_area, gle.subservice_area, gle.project, gle.donor, d.donor_type,
        ...
        LEFT JOIN `tabDonor` d ON gle.donor = d.name
        WHERE {conditions}
