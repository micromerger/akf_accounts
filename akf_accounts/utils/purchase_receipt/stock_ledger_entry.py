...
def update_stock_dimensions(doc, method=None):
	...
	for dimensions in self.custom_program_details:
		...
		fields_list += ["inventory_flag = 'Purchased' "]
			...
			if frappe.db.exists(
				...
				frappe.db.sql(
					...
					SET 
						...
						cost_center = '{get_cost_center(row.warehouse)}'
					WHERE
						docstatus = 1 
...
def get_cost_center(warehouse):
	cost_center = frappe.db.sql(f''' Select custom_cost_center From `tabWarehouse`  Where name = '{warehouse}' ''')
	if(cost_center):
		return cost_center[0][0]
