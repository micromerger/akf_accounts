...
function fetchWIPCompositeAssetPiItems(frm) {
    ...
    console.log('Fetching WIP composite asset items for target asset:', frm.doc.target_asset);

    // Show loading indicator
    frm.dashboard.set_headline_alert('Fetching WIP composite asset items...');
    ...
    frappe.call({
        ...
        callback: function(response) {
            ...
            if (response.message && response.message.length > 0) {
                console.log('Found', response.message.length, 'matching Purchase Invoice Items');
        ...
        error: function(err) {
            ...
            console.error('Error fetching WIP composite asset items:', err);
            frappe.msgprint({
                title: __('Error'),
                message: __('Error fetching WIP composite asset items. Please try again.'),
                indicator: 'red'
            });
        }
    });
